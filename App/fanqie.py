...
def get_every_chapter(bookurl:str):
    ...
    chapters = []
    temp_dict['bookname'] = soup.select("div.info-name h1")[0].text
    for chap in soup.select("a.chapter-item-title"):
        chapters.append(Chapter(
            chapter_link = "https://fanqienovel.com"+chap['href'],
            chapter_name = chap.text
        ))
    return chapters

def get_chapter_content(chapter:Chapter):
    soup = getSoup(chapter.chapter_link,info['driver'],0.6)
    contents = ""
    for p in soup.select("div.muye-reader-content div p"):
        for text in p.text:
            t = ''
            try:
                t = dict_data[str(ord(text))]
            except:
                t = text
            if t in html_tags:
                continue
